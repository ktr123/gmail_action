...
def validate_response_content(actual_response,
    ...
    list_compare(actual_response.keys(), expected_response.keys(),
                 key=response_key)
    for response_key in expected_response:
        if 'list' in str(type(expected_response[response_key])) \
                and expected_response[response_key] \
                and 'dict' in str(type(expected_response[response_key][0])):
            if not actual_response[response_key]:
                pytest.fail(
                    f"Incorrect Format for the key : {response_key} " 
                    f"Expected format : {expected_response[response_key]} "
                    f"Actual format : {actual_response[response_key]}")
